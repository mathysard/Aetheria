                    ...
                    <div className="flex justify-between mx-4">
                        <svg
                            ...
                            onClick={() => {
                                ...
                                (document.getElementById('characterUserFieldUpdate') as HTMLInputElement).value = "";
                            }}
                        >
                ...
                <div className="w-full mt-8 bg-white">
                    <div className="flex justify-between mx-4">
                        <svg
                            ...
                            onClick={() => {
                                dialogRef.current?.close();
                                (document.getElementById('locationUserFieldUpdate') as HTMLInputElement).value = "";
                            }}
                        >
                            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
                        </svg>
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            setLocations(prev => {
                                ...
                                foundLocation.imageName = file.name;
                                reader.onloadend = function() {
...
const UpdateBook = () => {
    ...
    const handleSubmit = () => {
        ...
        switch(activeScreen) {
            case "book":
                if(cover) {
                    formData.append('cover', cover, coverName)
                }
            ...
            case "characters":
                characters.map(character => {
                    if(character.image && character.imageBase64) {
                        formData.append(character.uuid, character.image, character.imageName);
                    }
                })
            ...
            case "locations":
                locations.map(location => {
                    if(location.image && location.imageBase64) {
                        formData.append(location.uuid, location.image, location.imageName);
                    }
                })
        ...
        fetch(`https://127.0.0.1:8000/api/updateBook/${bookId}/${activeScreen}`, {
            method: 'POST',
            body: formData
        })
    ...
    document.onkeydown = (e: any) => {
        if(['characters', 'locations'].includes(activeScreen)) {
            ...
            if(e.key === 'Escape') {
                (document.getElementById(`${fieldId}UserFieldUpdate`) as HTMLInputElement).value = '';
            }
        }
    }
    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            if(res.result === true) {
                ...
                let coverType = book.cover.split('.');
                coverType = coverType[coverType.length - 1];

                if(book.cover) {
                    setCover(base64ToBlob(book.coverBase64.split(";base64,")[1], "image/" + coverType))
                    setCoverBase64(book.coverBase64);
                    setCoverName(book.cover);
                }
                setCharacters(() => {
                    const newCharacters = res.characters;
                    newCharacters.map((char: any, i: number) => {
                        let characterImageType = char.image.split('.');
                        characterImageType = characterImageType[characterImageType.length - 1];
                        newCharacters[i].image = base64ToBlob(char.imageBase64.split(";base64,")[1], "image/" + characterImageType);
                    });
                ...
                setLocations(() => {
                    ...
                    newLocations.map((loc: any, i: number) => {
                        let locationImageType = loc.image.split('.');
                        locationImageType = locationImageType[locationImageType.length - 1];
                        newLocations[i].image = base64ToBlob(loc.imageBase64.split(";base64,")[1], "image/" + locationImageType);
                    });
                ...
                <div className="flex">
                    ...
                    <div className="ml-[8%] px-8 pt-4 bg-white shadow-xl">
                        <div className="flex h-max border-b-2 mb-4 border-gray-400">
                            <p
                                className={`text-2xl font-semibold mb-4 mr-16 ${activeScreen === "book" ? "text-blue-400" : ""} ${fetchIsActive ? "cursor-not-allowed" : "cursor-pointer"} ${fetchIsActive ? "hover:text-red-400" : "hover:text-blue-400"} ${fetchIsActive ? "active:text-red-600" : "active:text-blue-600"}`}
                                onClick={() => {
                            ...
                            <p
                                className={`text-2xl font-semibold mb-4 mr-16 ${activeScreen === "characters" ? "text-blue-400" : ""} ${fetchIsActive ? "cursor-not-allowed" : "cursor-pointer"} ${fetchIsActive ? "hover:text-red-400" : "hover:text-blue-400"} ${fetchIsActive ? "active:text-red-600" : "active:text-blue-600"}`}
                                onClick={() => {
                            ...
                            <p
                                className={`text-2xl font-semibold mb-4 mr-16 ${activeScreen === "relations" ? "text-blue-400" : ""} ${fetchIsActive ? "cursor-not-allowed" : "cursor-pointer"} ${fetchIsActive ? "hover:text-red-400" : "hover:text-blue-400"} ${fetchIsActive ? "active:text-red-600" : "active:text-blue-600"}`}
                                onClick={() => {
                            ...
                            <p
                                className={`text-2xl font-semibold mb-4 mr-16 ${activeScreen === "locations" ? "text-blue-400" : ""} ${fetchIsActive ? "cursor-not-allowed" : "cursor-pointer"} ${fetchIsActive ? "hover:text-red-400" : "hover:text-blue-400"} ${fetchIsActive ? "active:text-red-600" : "active:text-blue-600"}`}
                                onClick={() => {
                            ...
                            <p
                                className={`text-2xl font-semibold mb-4 ${activeScreen === "chapters" ? "text-blue-400" : ""} ${fetchIsActive ? "cursor-not-allowed" : "cursor-pointer"} ${fetchIsActive ? "hover:text-red-400" : "hover:text-blue-400"} ${fetchIsActive ? "active:text-red-600" : "active:text-blue-600"}`}
                                onClick={() => {
